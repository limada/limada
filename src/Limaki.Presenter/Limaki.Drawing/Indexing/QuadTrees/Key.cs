...
namespace Limaki.Drawing.Indexing.QuadTrees {
    ...
    public class Key {
        ...
        private RectangleD _env = default(RectangleD);
        public virtual RectangleD Envelope {
            get { return _env; }
        }


        public Key ( RectangleD itemEnv ) {
            ComputeKey(itemEnv);
        }
        ...
        public static int ComputeQuadLevel ( RectangleD env ) {
            double dx = env.Width;
        ...
        public virtual void ComputeKey ( RectangleD itemEnv ) {
        ...
        private void ComputeKey ( int level, RectangleD itemEnv ) {
            double quadSize = DoubleBits.PowerOf2(level);
            _pt.X = (double) ( Math.Floor(itemEnv.X / quadSize) * quadSize );
            _pt.Y = (double) ( Math.Floor(itemEnv.Y / quadSize) * quadSize );
            _env = new RectangleD(_pt.X, _pt.Y, (double) quadSize, (double) quadSize);
        }
    }
}
