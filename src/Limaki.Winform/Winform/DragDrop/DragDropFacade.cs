...
namespace Limaki.Winform.DragDrop {
    ...
    public class DragDropFacade {
        ...
        public IWidget PlaceWidget(IDataObject dataObject, Scene scene, ILayout<Scene, IWidget> layout) {
            IWidget widget = GetWidget (dataObject, scene.Graph,false);
            SceneTools.PlaceWidget(scene.Focused,widget,scene,layout);
       ...
       public virtual bool DoDragDrop(Scene scene, IControl control, IDataObject dataObject, ILayout<Scene, IWidget> layout, PointI pt, int hitsize) {
            IWidget item = null;
            ...
            if (dataObject is ControlDataObject) { // data is sent from same application
                ...
                // data is sent from another control
                else if (data.control is IDataDisplay<Scene>) {// the other control has a Scene
                    ...
                    IGraphPair<IWidget, IWidget, IEdgeWidget, IEdgeWidget> sourceGraph =
                        ((IDataDisplay<Scene>)data.control).Data.Graph as IGraphPair<IWidget, IWidget, IEdgeWidget, IEdgeWidget>;
