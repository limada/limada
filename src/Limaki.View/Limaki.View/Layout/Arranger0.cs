...
namespace Limaki.View.Layout {
    ...
    public class Arranger0<TItem, TEdge>
        ...
        public IGraphSceneLayout<TItem, TEdge> Layout {
            ...
            protected set {
                if (_layout != value) {
                    ...
                    this.Dimension = _layout.Dimension;
        ...
        protected virtual void AdjustRowLocation(Row<TItem> row, Point location) {
            ...
            if (Dimension == Drawing.Dimension.Y) {
            ...
            if (Centered) {
                ...
                } else if (Dimension == Drawing.Dimension.X) {
        ...
        protected void FirstFreeRowCollissionResolver(Row<TItem> row, ICollection<TItem> ignoring) {
            ...
            Rectangle stripe = new Rectangle(row.Location, row.Size);
            var startStripe = stripe;

            Func<Rectangle, double> orderByR = r => r.Right;
            Func<Rectangle, double> orderByB = r => r.Bottom;
            Func<Rectangle, double> orderBy = orderByR;
            Action<Rectangle> moveDown = (r) => { stripe.Y = r.Bottom + Distance.Height; };
            Action<Rectangle> moveRight = (r) => { stripe.X = r.Right + Distance.Width; };

            if (Dimension == Drawing.Dimension.X) {
            ...
            while (collission && iterations > 0) {
                ...
                var l = from item in Data.ElementsIn(stripe)
                        ...
                        select new Rectangle(Proxy.GetLocation(item), Proxy.GetSize(item));
        ...
        public virtual void Commit() {
            Proxy.Commit(this.Data.Requests);
        }
        ...
        protected virtual Pair<Point, Point> StripeLocactions(Rectangle stripe, Row<TItem> row, ICollection<TItem> ignoring) {
            if (Dimension == Drawing.Dimension.Y) {
