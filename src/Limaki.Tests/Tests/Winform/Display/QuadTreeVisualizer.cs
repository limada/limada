    ...
    public partial class QuadTreeVisualizer : Form {
        ...
        private void PopulateDisplay () {
            ...
            if ( Data != null ) {
                ...
                IStyle style = widgetDisplay.DataLayout.StyleSheet[StyleNames.DefaultStyle];
        ...
        private IWidget NodeWidget ( Node<IWidget> node,
            ...
            if ( node != null ) {
                IWidget result = factory.CreateWidget(
                ...
                if ( node.HasSubNodes ) {
                    IWidget subRoot = factory.CreateWidget(" ° ");
                    this.widgetDisplay.Data.Add(
                        factory.CreateEdgeWidget(string.Empty, result, subRoot));

                    foreach (Node<IWidget> sub in node.Subnodes) {
                        if (sub != null) {
                            IWidget subWidget = NodeWidget(sub, nodesDone, itemsDone);
                            this.widgetDisplay.Data.Add(
                                factory.CreateEdgeWidget(string.Empty,
                                                       subRoot, subWidget));
                        }
                    }
                }
            ...
            foreach ( IWidget widget in node.Items ) {
                ...
                if ( childWidget == null ) {
                    ...
                    if ( !node.Envelope.Contains(widget.Shape.BoundsRect) )
                        ds = "! " + ds;
                    childWidget = factory.CreateWidget(ds);
                    childWidget.Shape =
                ...
                IEdgeWidget edge = 
                    factory.CreateEdgeWidget(string.Empty, childWidget, nodeWidget);
