...
namespace Limaki.Tests.Widget {
    public class BenchmarkOneTests:WidgetDisplayTest {
        ...
        public override void Setup() {
            if (Display != null) {
                oldlayout = ( (GDISceneControler<Scene, IWidget>) Display.LayoutControler ).Layout;
                ( (GDISceneControler<Scene, IWidget>) Display.LayoutControler ).Layout = null;
            }
            base.Setup();
            // this is neccessary as the mouse cursor returns after a long time
            // back to its position and activates WidgetTextEditor
            ((GDISceneControler<Scene, IWidget>)Display.LayoutControler).Layout =
                new BenchmarkOneSceneFactory.LongtermPerformanceLayout(
                    Display.DisplayKit.dataHandler,factory.styleSheet);
            ( (GDIWidgetLayer) Display.DataLayer ).Layout =
                ( (GDISceneControler<Scene, IWidget>) Display.LayoutControler ).Layout;
        ...
        public void MoveNode1(RectangleI bounds) {
            ...
            MouseActionEventArgs e = new MouseActionEventArgs(MouseActionButtons.Left, 0, position.X, position.Y, 0);
            ...
            e = new MouseActionEventArgs(MouseActionButtons.Left, 0, position.X, position.Y, 0);
        ...
        public void MoveAlongSceneBoundsTest() {
            ...
            this.ReportDetail ("Scene:\t" + bounds+"\t DisplaySize: \t"+Display.Bounds);
            MoveNode1 (bounds);
