...
namespace Limaki.Graphs.Extensions {
    public class GraphPairFacade<TItem, TEdge>
        ...
        public IGraphPair<TItem, TTwo, TEdge, TEdgeTwo> Source<TTwo, TEdgeTwo>(IGraph<TItem, TEdge> graph)
            where TEdgeTwo : IEdge<TTwo>, TTwo {
            
            var result = graph as IGraphPair<TItem, TItem, TEdge, TEdge>;
            
            if (result != null) {
                result = this.Source(graph);
        ...
        public IGraphPair<TItem, TItem, TEdge, TEdge> Source(IGraph<TItem, TEdge> graph) {
            var result = graph as IGraphPair<TItem, TItem, TEdge, TEdge>;
            if (result != null) {
                while (result.Two is IGraphPair<TItem, TItem, TEdge, TEdge>) {
                    result = (IGraphPair<TItem, TItem, TEdge, TEdge>)result.Two;
        ...
        public IEnumerable<IGraph<TItem, TEdge>> Graphs(IGraph<TItem,TEdge> graph) {
        ...
        public TItem LookUp<TTwo, TEdgeTwo>(
        ...
        public virtual int InEdgeCount(IGraph<TItem, TEdge> graph, TItem item) {
            int result = 0;
        ...
        public virtual IEnumerable<TItem> FindRoots(IGraph<TItem, TEdge> graph, TItem focused) {
            if (graph != null) {
        ...
        public virtual void PopulateWithRoots<TTwo, TEdgeTwo>(GraphView<TItem, TEdge> graphView)
            where TEdgeTwo : IEdge<TTwo>, TTwo {
            IGraphPair<TItem, TTwo, TEdge, TEdgeTwo> source = Source<TTwo, TEdgeTwo>(graphView);
            if (source != null) {
                foreach (TTwo item in new GraphPairFacade<TTwo, TEdgeTwo>().FindRoots(source.Two, default(TTwo))) {
