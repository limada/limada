...
namespace Limaki.Tests.Drawing {
    ...
    public class VectorHulltest : DomainTest {
        public enum Algo {
            AtanSinCos,
            Transform,
            SqareRoot
        ...
        public Point[] VectorHullAtanSinCos (Vector _data, int width) {
        ...
        public virtual Point[] VectorHullTransform (Vector _data, double delta) {
            Point[] line = null;
        ...
        public virtual Point[] VectorHullSqareRoot (Vector _data, int delta) {
            // get it near:
        ...
        void PaintWidenVector (Algo algo, Point start, Size size, int i) {
            if (algo == Algo.AtanSinCos)
                VectorHullAtanSinCos(new Vector(start, start + size), 10);
            else if (algo == Algo.SqareRoot)
                VectorHullSqareRoot(new Vector(start, start + size), 10);
            else if (algo == Algo.Transform)
                VectorHullTransform(new Vector(start, start + size), 10);
        }
        ...
        public void TestCases (Algo algo) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                PaintWidenVector(algo, start, size, 1);

                start += distance;
                size = new Size(0, 100);
                PaintWidenVector(algo, start, size, 2);

                start += distance;
                size = new Size(100, 0);
                PaintWidenVector(algo, start, size, 3);

                start += distance;
                size = new Size(-100, 0);
                PaintWidenVector(algo, start, size, 4);

                // second

                start += distance;
                size = new Size(10, -100);
                PaintWidenVector(algo, start, size, 5);

                start += distance;
                size = new Size(10, 100);
                PaintWidenVector(algo, start, size, 6);

                start += distance;
                size = new Size(100, 10);
                PaintWidenVector(algo, start, size, 7);

                start += distance;
                size = new Size(-100, 10);
                PaintWidenVector(algo, start, size, 8);

                // third

                start += distance;
                size = new Size(-10, -100);
                PaintWidenVector(algo, start, size, 9);

                start += distance;
                size = new Size(-10, 100);
                PaintWidenVector(algo, start, size, 10);

                start += distance;
                size = new Size(100, -80);
                PaintWidenVector(algo, start, size, 11);

                start += distance;
                size = new Size(-100, -10);
                PaintWidenVector(algo, start, size, 12);
            }
            ticker.Stop();
        ...
        [Test]
        public void TestVectorHull () {
            this.ReportDetail("VectorHullTest");
