...
namespace Limaki.View.Xwt.Visuals {
    ...
    public class VisualsXwtragDrop : MouseDragActionBase, IXwtDragDropAction, IKeyAction, IDragDopActionPresenter {
        ...
        public override void OnMouseMove (MouseActionEventArgs e) {
            ...
            if (Resolved && !Dragging) {
                Dragging = true;
                try {
                    var myDataObject = DragDrop.SetVisual(this.control, Scene.Graph, Current);
                    var dropEffect =
        ...
        public virtual void OnDragDrop (DragEventArgs e) {
            ...
            if (!DragDrop.DoDragDrop(this.Scene, this.control, e.Data.ToXwt(), this.Layout, pt, this.HitSize)) {
        ...
        public virtual void OnDragOver (DragEventArgs e) {
            if (!DragDrop.IsValidData(e.Data.ToXwt())) {
                e.Effect = DragDropEffects.None;
                return;
            }
            ...
            var sourceIsVisualControl = e.Data is VidgetBackendTransferDataSource;
        ...
        public virtual void Paste () {
            if (this.Scene == null)
                return;
            var visual = DragDrop.PlaceVisual(Clipboard.GetDataObject().ToXwt(), this.Scene, this.Layout);
