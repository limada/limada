...
namespace Limaki.Presenter.Winform.Widgets {
    ...
    /// </summary>
    public class WidgetTextEditor:MouseTimerActionBase, IKeyAction, IEditAction {
        ...
        public WidgetTextEditor(
            ...
            IDisplay display,
            ICamera camera,
            IGraphLayout<IWidget,IEdgeWidget> layout)
            : this() {
            this.device = device;
        ...
        Get<IGraphScene<IWidget, IEdgeWidget>> _sceneHandler;
        ...
        void DetachEditor(bool writeData) {
            ...
            if (focusAfterEdit) {
                ...
                if (scene.Focused != null)
                    scene.Requests.Add(new StateChangeCommand<IWidget>(scene.Focused, new Pair<UiState>(UiState.Selected, UiState.None)));
        ...
        public void OnKeyDown( KeyActionEventArgs e ) {
            ...
            if (insert) {
                DetachEditor(true);
                Exclusive = Resolved = true;
                Current = Registry.Pool.TryGetCreate<IWidgetFactory>().CreateItem<string>("XXXXXXXX");
                var scene = Scene as Scene;
