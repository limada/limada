...
using Limada.Model;
using Limaki.App.StreamViewers.WebProxyServer;
...
namespace Limaki.App.StreamViewers {
    public class HTMLViewerController : StreamViewerController {
        ...
        public virtual void SetProxy() {
            var control = Control as Limaki.ThirdPartyWrappers.GeckoWebBrowser;
            if (control != null){
                var prefs = Skybound.Gecko.GeckoPreferences.User;

                prefs["network.proxy.http"] = webServer.Addr.ToString();
                prefs["network.proxy.http_port"] = webServer.Port;
        ...
        public override void SetContent(StreamInfo<Stream> info) {
            ...
            try {
                IWebBrowser webBrowser = Control as IWebBrowser;
                if (UseWebServer || UseProxy) {
                    ...
                    using (StreamReader reader = new StreamReader(info.Data)) {
                        ...
                        ((IWebBrowser)Control).DocumentText = text;
        ...
        public override void Clear() {
            ...
            if (_control != null) {
                var control = _control as IWebBrowser;
